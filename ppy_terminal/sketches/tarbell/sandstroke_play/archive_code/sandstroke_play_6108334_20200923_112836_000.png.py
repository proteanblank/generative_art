...
class SandPainter:
  ...
  def render(self, x, y, ox, oy):
    ...
    pushStyle()
    strokeWeight(2)
    stroke(self.h, self.s, self.b, 50) #*a
    for i in range(5):
      pct = map(i, 0, 5, 0, 2)
      amt = noise((x+y+frameCount)*0.02)*pct
      point(lerp(x, (x+ox)/2, amt), lerp(y, (y+oy)/2, amt))
      point(lerp(ox, (x+ox)/2, amt), lerp(oy, (y+oy)/2, amt))
